...
export default class Slider extends Module {
  ...
  cacheDom() {
    return {
      ...
      slideHero: this.el.querySelectorAll(".Slider-hero")
    };
  }
    ...
    if (this.settings.slidesToShow) {
      for (let i = 0; i < this.dom.slide.length; i++) {
        if (this.settings.slidesToShow == 1) {
          this.dom.slide[i].style.flex = "0 0 100%";
          this.dom.slide[i].style.width = "100%";
        } else if (this.settings.slidesToShow == 2) {
  ...
  changeSlide(sliderNum, direction) {
    ...
    if (this.settings.slidesToShow) {
      ...
      // Slide active state
      for (let i = 0; i < slidesTotal; i++) {
        this.dom.slide[i].classList.remove("Active");
      }
      ...
      // target hero
      for (let i = 0; i < slidesTotal; i++) {
        this.dom.slideHero[i].classList.remove("Active");
      }

      this.dom.slideHero[sliderNum].classList.add("Active");
    }
